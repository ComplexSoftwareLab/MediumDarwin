...
class JavaMutate(object):
    ...
    def gatherMutations(
        ...
        # First try with the file name as-is
        file_results = database.fetch_data(
            "file", columns="id", condition=f"name = '{self.file_name}'"
        )
        if not file_results:
            # Try with normalized path (forward slashes, relative path)
            # Note: We don't have access to buildPath here, so we try basic normalization
            normalized_name = self.file_name.replace("\\", "/")
            if normalized_name != self.file_name:
                file_results = database.fetch_data(
                    "file", columns="id", condition=f"name = '{normalized_name}'"
                )
        if not file_results:
            raise ValueError(
                f"File not found in database: {self.file_name}. "
                f"Make sure the file was inserted into the database during the coverage phase."
            )
        ...
        sorted_operators = sorted(
            ...
            key=lambda mO: mO.mutatorType
        )
        for mO in sorted_operators:
            for metaType in metaTypes:
                if metaType in mO.metaTypes:
                    mutationTypeCount[mO.mutatorType] = len(mO.mutations)
                    # Sort mutations within each operator for deterministic ordering
                    sorted_mutations = sorted(
                        ...
                        key=lambda m: (
                            ...
                            m.nodeID,
                            m.mutatorType,
                            m.mutationID if m.mutationID is not None else 0
                        )
                    )
